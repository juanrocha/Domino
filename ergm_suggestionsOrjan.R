...
df_test <- data_frame(paths = as.sociomatrix(x, 'paths')[upper.tri(as.sociomatrix(x))])
...
fit1 <- lm(paths ~ landuse + ecotype + ecoprocess + prov_service + reg_service + 
               cult_service + hwb + space_scale + time_scale + reversibility + evidence,
           data = df_test)

### ERGMS for Regime Shifts
# library(ergm.count)

fit.null1 <- suppressMessages(
    ergm(x ~ nonzero + sum , 
         response ='paths', reference=~Poisson, 
         control = control.ergm(MCMLE.trustregion=1000))) ## AIC: -1258


fit.w1 <- suppressMessages(
    ergm(x ~ nonzero + sum + edgecov(x,'landuse') + edgecov(x,'ecotype')+
             edgecov(x,'ecoprocess') + edgecov(x,'reg_service') + 
             edgecov(x,'prov_service') + edgecov(x,'cult_service') +  
             edgecov(x,'hwb') + nodefactor('space_range') + nodefactor('time_range') +
             nodematch('space_range', diff = FALSE)+ nodematch('time_range', diff = FALSE)+
             edgecov(x,'reversibility') + edgecov(x,'evidence'), 
         response ='paths', reference = ~Poisson, control = control.ergm(MCMLE.trustregion=1000)))
# + nodemix('space_range', base = 1) + nodemix("time_range", base = 1) gives a degenerated results.

fit.w1a <- suppressMessages(
    ergm(x ~ nonzero + sum + edgecov(x,'landuse') +
             edgecov(x,'ecotype')+ edgecov(x,'ecoprocess') + edgecov(x,'reg_service') +
             edgecov(x,'prov_service') + edgecov(x,'cult_service') +  edgecov(x,'hwb') + 
             edgecov(x,'space_scale')+ edgecov(x,'time_scale') + edgecov(x,'reversibility') +
             edgecov(x,'evidence'), response ='paths',
         reference = ~Poisson, control = control.ergm(MCMLE.trustregion=1000)))

fit.w1b <- suppressMessages(
    ergm(x ~ nonzero + sum + transitiveties(threshold = 0) +  # mutual() + ## mutual only works on directed networks
             edgecov(x,'landuse') + edgecov(x,'ecotype')+ edgecov(x,'ecoprocess') + edgecov(x,'reg_service') +
             edgecov(x,'prov_service') + edgecov(x,'cult_service') +  edgecov(x,'hwb') + 
             edgecov(x,'space_scale')+ edgecov(x,'time_scale') + edgecov(x,'reversibility') +
             edgecov(x,'evidence'), response ='paths',
         reference = ~Poisson, control = control.ergm(MCMLE.trustregion=1000)))
