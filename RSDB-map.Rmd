...
coord_rs <- read.csv(file = '~/Documents/Projects/Cascading Effects/Domino/rs_coords_paper.csv', dec = '.')
...
### Minimize the datasets to faciliate the interactive ploting. You don't need the full files of the database.
...
## Load urls dataset
load('~/Documents/Projects/accio/regimeShifts_url.RData')
...
df$name[!df$name %in% urls$case_names] <- urls$case_names[!urls$case_names %in% df$name][c(17:15,4,6,7,1,10,9,5,2,3,11)]
...
#     addPolygons(fillColor = "grey50", stroke = F) %>%
#     addCircles(data = cases, lat = ~lat, lng = ~lon, radius = 1, color = "orange")

pal <-  colorFactor(palette = "viridis", domain = levels(df$rs), alpha = FALSE, na.color = "#808080" )

# J180811: I don't think I need the world map per se.... with the coordinates it should know it's the world that I need.
...
## Error discovered: all urls from the database are wrong! fuck

```
J180822: There is a few errors on the chunk above because of the different versions of the file / database. The latest version does not have Primary production in the Arctic Ocean nor Desertification links. The code working is corrected in the blog post prepared for Rethink (website repo).
